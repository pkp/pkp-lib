...
class PKPInstall extends Installer
{
    ...
    public function preInstall()
    {
        ...
        // Warn (non-blocking) only about a non-UTF8 PostgreSQL database here. The
        // MySQL/MariaDB charset-mix check concerns *existing* tables, which is an
        // upgrade concern rather than a fresh install, so it is left to the upgrade
        // pre-flight. See pkp/pkp-lib#11563.
        if ($driver === PKPContainer::DRIVER_POSTGRES && ($warning = PKPContainer::getDatabaseCharsetWarning($driver))) {
            $this->log($warning);
        }
