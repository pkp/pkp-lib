...
{
    /** Log Viewer type identifier for the PKP application log */
    public const LOG_TYPE_APPLICATION = 'pkp_application';
    ...
    public function register()
    {
        ...
        // Bind a LogTypeRegistrar that recognises the PKP application log filename (and its
        // daily-rotation suffix) so it is parsed by PKPApplicationLog rather than the vendor's
        // LaravelLog. That parser reads both Monolog output formats, which matters because
        // [logs] log_formatter can switch the file to JSON mid-stream.
        $this->app->singleton(LogTypeRegistrar::class, function () {
            return new class (PKPContainer::LOG_FILE_NAME) extends LogTypeRegistrar {
                ...
                {
                    return $this->isPkpApplicationLogFile($file->name)
                        ? PKPLogViewerServiceProvider::LOG_TYPE_APPLICATION
                        : parent::guessTypeFromFileName($file);
                }

                private function isPkpApplicationLogFile(string $fileName): bool
                {
    ...
    protected function configureLogViewer(): void
    {
        $this->app->get('config')->set('log-viewer', [
            ...
            // UI settings
            // Suppress the vendor's upstream support/donate link in the PKP UI
            ...
            'include_files' => array_merge(
                [
                    // Application logs under files_dir/logs/
                    storage_path(PKPContainer::LOG_DIRECTORY) . '/*.log',
                ],
