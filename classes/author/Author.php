...
class Author extends Identity
{
    ...
    /**
     * Get contributor roles as assoc array [ identifier => translations ].
     */
    public function getContributorRoleNames(): array
    {
        return collect($this->getData('contributorRoles'))
            ->mapWithKeys(fn (ContributorRole $role) => [
                $role->contributor_role_identifier => $role->name
            ])
            ->toArray();
    }

    /**
     * Get localized contributor roles as assoc array [ identifier => localized translation ].
     */
    public function getLocalizedContributorRoleNames(?string $preferredLocale = null): array
    {
        return collect($this->getData('contributorRoles'))
            ->mapWithKeys(fn (ContributorRole $role) => [
                $role->contributor_role_identifier => $role->getLocalizedData('name', $preferredLocale)
            ])
            ->toArray();
    }
     ...
     * Use setContributorRolesByIdentifiers when using identifiers[]
     */
    ...
    /**
     * Set contributor roles using identifiers with context id.
     * Use setContributorRoles when using ContributorRole[]
     */
    public function setContributorRolesByIdentifiers(array $identifiers, int $contextId): void
    {
        $this->setData('contributorRoles', Repo::contributorRole()->getByContextIdAndIdentifiers($identifiers, $contextId));
    }
}
