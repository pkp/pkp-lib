...
class PKPPageRouterTest extends PKPRouterTestCase {
	...
	public function testUrlWithPathinfo() {
		...
		$result = $this->router->url($this->request, array('firstContext' => null), null, 'new-op', 'add-path');
		...
		self::assertEquals('http://mydomain.org/index.php/new-context1#some%3Fanchor', $result);
	...
	public function testUrlWithoutPathinfo() {
		...
		$result = $this->router->url($this->request, 'new-context1', null, null, null, null, 'some?anchor');
		self::assertEquals('http://mydomain.org/index.php?firstContext=new-context1#some%3Fanchor', $result);
