...
class PublicationTest extends PKPTestCase {
	...
	public function testPageArray() {
		...
		$this->publication->setPages('pg. i-ix, 6-11, 19, 21');
		$pageArray = $this->publication->getPageArray();
		$this->assertSame($expected,$pageArray);
		// no spaces
		$this->publication->setPages('i-ix,6-11,19,21');
		$pageArray = $this->publication->getPageArray();
		$this->assertSame($expected,$pageArray);
		// double-hyphen
		$this->publication->setPages('i--ix,6--11,19,21');
		...
		$this->publication->setPages('16');
		$pageArray = $this->publication->getPageArray();
		$this->assertSame($expected,$pageArray);
		// spaces in a range
		$expected = array(array('16', '20'));
		$this->publication->setPages('16 - 20');
		$pageArray = $this->publication->getPageArray();
		$this->assertSame($expected,$pageArray);
		// pages are alphanumeric
		$expected = array(array('a6', 'a12'), array('b43'));
		$this->publication->setPages('a6-a12,b43');
		$pageArray = $this->publication->getPageArray();
		$this->assertSame($expected,$pageArray);
		// inconsisent formatting
		$this->publication->setPages('pp:  a6 -a12,   b43');
		$pageArray = $this->publication->getPageArray();
		$this->assertSame($expected,$pageArray);
		$this->publication->setPages('  a6 -a12,   b43 ');
		$pageArray = $this->publication->getPageArray();
