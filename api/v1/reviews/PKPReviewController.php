...
use PKP\submission\reviewRound\RequestAuthorReviewResponseManager;
...
class PKPReviewController extends PKPBaseController
{
    ...
    {
        Route::post('authorResponse/{reviewRoundId}', $this->submitAuthorResponse(...))
            ->name('review.authorResponse.submitResponse')
            ->whereNumber(['reviewRoundId'])
            ->middleware([
                self::roleAuthorizer([
                    Role::ROLE_ID_AUTHOR,
                ])
        ...
        Route::put('authorResponse/{reviewRoundId}/{responseId}', $this->editAuthorResponse(...))
            ->name('review.authorResponse.submitResponse')
            ->whereNumber(['reviewRoundId', 'responseId'])
            ->middleware([
                self::roleAuthorizer([
        ...
        Route::post('authorResponse/{reviewRoundId}/requestResponse', $this->requestAuthorResponse(...))
        ...
        Route::get('authorResponse/{reviewRoundId}', $this->getAuthorResponse(...))
            ->name('review.authorResponse.get')
            ->whereNumber(['reviewRoundId'])
            ->middleware([
                self::roleAuthorizer([
                    Role::ROLE_ID_SITE_ADMIN,
        ...
        Route::delete('authorResponse/{reviewRoundId}', $this->deleteAuthorResponse(...))
    ...
    public function authorize(PKPRequest $request, array &$args, array $roleAssignments): bool
    {
        ...
        if (!in_array($actionName, ['submitAuthorResponse','requestAuthorResponse', 'editAuthorResponse', 'deleteAuthorResponse'])) {
    ...
    /**
     * Allow assigned author to submit review response.
     */
    public function submitAuthorResponse(Request $illuminateRequest): JsonResponse
    {
        ...
        if ($hasExistingResponse) {
            ...
            return response()->json(
                [
                    'error' => 'A response has already been submitted for this review round.'
                ],
                Response::HTTP_CONFLICT
            );
        }

        // check that review round is in state where author response is applicable(revisions required, submission accepted, or review response Requested)
        if (
            !in_array($reviewRound->getStatus(), [ReviewRound::REVIEW_ROUND_STATUS_REVISIONS_REQUESTED, ReviewRound::REVIEW_ROUND_STATUS_RESUBMIT_FOR_REVIEW])
    ...
    public function editAuthorResponse(Request $illuminateRequest): JsonResponse
    {
        ...
        $isAssignedAsManager = StageAssignment::withSubmissionIds([$reviewRound->getSubmissionId()])
    ...
    public function requestAuthorResponse(Request $illuminateRequest): JsonResponse
    {
        ...
        $requestAuthorResponse = new RequestAuthorReviewResponseManager(
        ...
        $requestAuthorResponse->sendMail(new EmailData($payload), $request->getUser());
    ...
    private function authorResponseCommonValidations(Request $illuminateRequest): Enumerable
    {
        ...
        if (!$authorResponse) {
            ...
            $error->put('message', '`authorReviewResponse is required'); // update to use locale
        ...
        if (empty($associatedAuthorIds)) {
            $error->put('code', Response::HTTP_BAD_REQUEST);
            $error->put('message', 'At least one associatedAuthorId is required'); // update to use locale
            return collect(['error' => $error]);
        }
