...
<div class="page page_section page_section_{$sectionPath|escape}">
	...
	{if $preprints|@count}
		<ul class="cmp_preprint_list">
			...
			{* Pagination *}
			{if $prevPage > 1}
				{capture assign="prevUrl"}{url router=PKP\core\PKPApplication::ROUTE_PAGE page="section" op="view" path=$sectionPath|to_array:$prevPage}{/capture}
			{elseif $prevPage === 1}
				{capture assign="prevUrl"}{url router=PKP\core\PKPApplication::ROUTE_PAGE page="section" op="view" path=$sectionPath}{/capture}
			{/if}
			{if $nextPage}
				{capture assign="nextUrl"}{url router=PKP\core\PKPApplication::ROUTE_PAGE page="section" op="view" path=$sectionPath|to_array:$nextPage}{/capture}
			{/if}
			{include
