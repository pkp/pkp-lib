...
git diff --name-only --cached | xargs ./lib/vendor/bin/php-cs-fixer fix --allow-risky=yes --path-mode=intersection --config=.php_cs -q

# Run php-cs-fixer again with --dry-run to throw an error if any files could not be automatically formatted
git diff --name-only --cached | xargs ./lib/vendor/bin/php-cs-fixer fix --allow-risky=yes --dry-run --path-mode=intersection --config=.php_cs

if [ $? -eq 0 ]
then
	echo -e "\n\e[32m✔ Files formatted successfully.\e[0m\n"
	git diff --name-only --cached | xargs git add -u
else
