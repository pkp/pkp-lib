...
<form class="pkp_form" id="promote" method="post" action="{url op=$saveFormOperation}" >
	...
	<div id="availableFiles" style="margin-top: 30px;">
		...
		{elseif $stageId == $smarty.const.WORKFLOW_STAGE_ID_EDITING}
			{url|assign:filesToPromoteGridUrl router=$smarty.const.ROUTE_COMPONENT component="grid.files.copyedit.SelectableCopyeditedFilesGridHandler" op="fetchGrid" submissionId=$submissionId stageId=$stageId escape=false}
		{/if}
