...
class PKPPageRouterTest extends PKPRouterTest {
	...
	public function testUrlWithPathinfo() {
		...
		self::assertEquals('http://mydomain.org/index.php/new-context1/current-context2?key1=val1%3F&key2%5B%5D=val2-1&key2%5B%5D=val2%3F2', $result);
	...
	public function testUrlWithoutPathinfo() {
		...
		$result = $this->router->url($this->request, 'new-context1', null, null, 'add?path');
		self::assertEquals('http://mydomain.org/index.php?firstContext=new-context1&secondContext=current-context2&page=index&op=index&path%5B%5D=add%3Fpath', $result);

		$result = $this->router->url($this->request, 'new-context1', null, null, array('add-path1', 'add?path2'));
		self::assertEquals('http://mydomain.org/index.php?firstContext=new-context1&secondContext=current-context2&page=index&op=index&path%5B%5D=add-path1&path%5B%5D=add%3Fpath2', $result);
		...
		self::assertEquals('http://mydomain.org/index.php?firstContext=new-context1&secondContext=current-context2&key1=val1%3F&key2%5B%5D=val2-1&key2%5B%5D=val2%3F2', $result);
		...
		$result = $this->router->url($this->request, 'new-context1', null, 'new-op', 'add-path', array('key' => 'val'), 'some-anchor');
		self::assertEquals('http://mydomain.org/index.php?firstContext=new-context1&secondContext=current-context2&page=index&op=new-op&path%5B%5D=add-path&key=val#some-anchor', $result);
