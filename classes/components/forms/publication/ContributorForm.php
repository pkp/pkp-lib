...
class ContributorForm extends FormComponent
{
    ...
    public function __construct(string $action, array $locales, ?Submission $submission, Context $context)
    {
        ...
        $contributorRoles = collect(Repo::contributorRole()->getByContextId($context->getId(), Locale::getLocale()))
            ->map(fn (?string $translation, string $identifier): array => ['value' => $identifier, 'label' => $translation ?? $identifier])
            ->values()
            ->toArray();
        ...
        $this->addField(new FieldText('organizationName', [
            'label' => __('user.organizationName'),
        ...
        $this->addField(new FieldText('rorId', [
                'label' => __('user.rorId'),
                'showWhen' => $showWhenOrganization,
                'description' => __('user.rorId.description'),
        ]));
