...
{
    public const CONTRIBUTOR_TYPE_PERSON_FORM_FIELDS = [
        'givenName', 'familyName', 'preferredPublicName', 'email', 'country',
        'rorId', 'url', 'biography', 'affiliations',
    ];

    public const CONTRIBUTOR_TYPE_ORGANIZATION_FORM_FIELDS = [
        'organizationName', 'email', 'country', 'rorId',
        'url', 'biography', 'affiliations',
    ];

    public const CONTRIBUTOR_TYPE_ANONYMOUS_FORM_FIELDS = ['email', 'country'];
    ...
    /**
     * Get contributor roles as assoc array [ identifier => translation(s) ].
     * To get ContributorRole[] use getData
     */
    public function getContributorRoles(?string $locale = null): array
    {
     ...
     * @param ContributorRole[]|string[]|null $roles, either ContributorRole or identifiers
     */
    public function setContributorRoles(?array $roles, ?int $contextId = null): void
    {
        $this->setData('contributorRoles', !$contextId
            ? $roles
            : ContributorRole::query()
                ->withContextId($contextId)
                ->withIdentifiers($roles)
                ->get()
                ->all()
        );
    }
