...
class Repository
{
     ...
     */
    public function isCitationStructured(Citation $citation): bool
    {
        foreach ($citation->requirementIsStructured as $set) {
            $isStructured = false;

            foreach ($set as $item) {
                if (!empty($citation->getData($item))) {
                    $isStructured = true;
                    break;
                }
            }
     ...
     */
    public function importAdditionalCitations(int $publicationId, ?string $rawCitationList): ?array
    {
        $citationTokenizer = new CitationListTokenizerFilter();
        $citationStrings = $rawCitationList ? $citationTokenizer->execute($rawCitationList) : [];
    ...
    /**
     * Add a new job for metadata lookup
     */
    public function addJob(int $publicationId): void
    {
        ...
        foreach ($citations as $citation) {
            if ($citation->getData('isProcessed')) {
                ...
                dispatch(new MetadataLookup($citation->getId()))->delay(now()->addSeconds(60));
            }
        }
    }
}
