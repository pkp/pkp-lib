...
trait HasContextIdentityMetadata
{
    ...
    /**
     * Get the stamped abbreviation in the current locale, falling back to the live context
     * abbreviation or acronym when nothing has been stamped.
     */
    public function getLocalizedContextAbbreviation(Context $context): ?string
    {
        return $this->getLocalizedData('contextAbbreviation')
            ?: $context->getLocalizedData('abbreviation')
            ?: $context->getLocalizedData('acronym')
            ?: null;
    }

    /**
     * Get the stamped abbreviation in the given locale, falling back to the live context
     * abbreviation or acronym in that locale when nothing has been stamped.
     */
    public function getContextAbbreviation(string $locale, Context $context): ?string
    {
        return $this->getData('contextAbbreviation', $locale)
            ?: $context->getData('abbreviation', $locale)
            ?: $context->getData('acronym', $locale)
            ?: null;
    }
