...
abstract class I8933_EventLogLocalized extends Migration
{
    ...
    public function up(): void
    {
        ...
        $whenRules = $contexts
            ->map(fn (object $context) =>
                'WHEN ' . (int) $context->{$this->getContextIdColumn()} . ' THEN ' . DB::getPdo()->quote($context->primary_locale ?? $sitePrimaryLocale)
            )
            ->join("\n");
        ...
        DB::table('event_log_settings AS es')
            ...
            // If the join fails, we'll assume the submission file was removed
            ->leftJoin('submissions AS s', fn (JoinClause $j) =>
                $j->whereRaw(
    ...
    protected function renameSettings()
    {
        ...
        foreach ($this->mapSettings() as $eventType => $settings) {
            foreach ($settings as $oldSettingName => $newSettingName) {
                DB::table('event_log_settings', 'els')
