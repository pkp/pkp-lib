	...
	{fbvFormSection}
		{if $changeEmailPending}
			<p>
				...
				<button type="submit" name="action" value="cancelPendingEmail">Cancel</button>
			</p>
		{/if}
		{fbvElement type="email" disabled=$changeEmailPending|default:false label="user.email" id="email" value=$email size=$fbvStyles.size.MEDIUM required=true}
