...
class HelpHandler extends Handler {
	...
	function index($args, $request) {
		...
		if (!$isPluginHelpRequest || !file_exists($this->getPathForPluginHelpChapter($request, $language))) {
		...
		HookRegistry::call('Plugins::Help', array(&$pluginHelpChapters));
		...
		if (preg_match_all('/\(([^)]+\.md)\)/sm', file_get_contents($path . $language . '/SUMMARY.md'), $matches)) {
			...
			// Add the plugin help chapters to the chapters list
			foreach ($pluginHelpChapters as $key => $value) {
		...
		if (!$isPluginHelpRequest) {
			...
			if (preg_match('#../SUMMARY\.md#', $filename)) {
				...
				foreach (array_keys($pluginHelpChapters) as $key) {
					$content .= "   * [$key]($pluginHelpChapters[$key])";
				}
			}
		} else {
			$content = file_get_contents($this->getPathForPluginHelpChapter($request, $language));
		}
	 ...
	 */
	function getPathForPluginHelpChapter($request, $language) {
		$language = AppLocale::getIso1FromLocale(AppLocale::getLocale());
		$reqArgs = $request->getRequestedArgs();
		$pathParts = array_slice($reqArgs, 1);
		array_splice($pathParts, count($pathParts) - 1, 0, $language);
