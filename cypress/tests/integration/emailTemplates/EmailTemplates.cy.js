...
describe('Email Template Access Tests', function() {
	it('Remove unrestricted from a template', () => {
		...
		cy.openEmailTemplate('Submission Accepted', 'Submission Accepted');
	...
	it('Creates template with user groups assigned (restricted template)', () => {
		...
		// Select the mailable
		cy.contains('li.listPanel__item', 'Submission Accepted')
			.find('button')
			.contains('Edit')
			.click();
		...
		cy.openEmailTemplate('Submission Accepted', templateName);
		cy.get('input[name="assignedUserGroupIds"]').eq(1).should('be.checked');
