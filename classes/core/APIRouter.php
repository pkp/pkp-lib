...
class APIRouter extends PKPRouter
{
    ...
    public function url(
        ...
        // Assemble URL
        //
        $pathInfoArray = ['api', Application::API_VERSION, $endpoint];

        if ($this->validatePluginOptions($request, $pluginOptions)) {
            $pathInfoArray = array_merge([
                $this->getPluginApiPathPrefix(), 
                ...array_values($pluginOptions)
            ], $pathInfoArray );
        }
        $pathInfoArray = array_merge($context, $pathInfoArray);
