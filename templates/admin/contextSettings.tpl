...
{block name="page"}
	...
	<tabs :track-history="true">
		<tab id="setup" label="{translate key="manager.setup"}">
			<tabs :is-side-tabs="true" :track-history="true">
				...
				<tab id="languages" label="{translate key="common.languages"}">
					{capture assign=languagesUrl}{url router=PKPApplication::ROUTE_COMPONENT context=$editContext->getPath() component="grid.settings.languages.ManageLanguageGridHandler" op="fetchGrid" escape=false}{/capture}
				...
				<tab id="restrictBulkEmails" label="{translate key="admin.settings.restrictBulkEmails"}">
					{if $bulkEmailsEnabled}
						...
						/>
					{else}
						{capture assign="siteSettingsUrl"}{url router=PKPApplication::ROUTE_PAGE page="admin" op="settings" anchor="setup/bulkEmails"}{/capture}
		...
		<tab id="plugins" label="{translate key="common.plugins"}">
			<tabs :track-history="true">
				<tab id="installed" label="{translate key="manager.plugins.installed"}">
					{capture assign=pluginGridUrl}{url router=PKPApplication::ROUTE_COMPONENT context=$editContext->getPath() component="grid.settings.plugins.SettingsPluginGridHandler" op="fetchGrid" escape=false}{/capture}
					{load_url_in_div id="pluginGridContainer" url=$pluginGridUrl}
				</tab>
				<tab id="gallery" label="{translate key="manager.plugins.pluginGallery"}">
					{capture assign=pluginGalleryGridUrl}{url router=PKPApplication::ROUTE_COMPONENT context=$editContext->getPath() component="grid.plugins.PluginGalleryGridHandler" op="fetchGrid" escape=false}{/capture}
		...
		<tab id="users" label="{translate key="manager.users"}">
			{capture assign=usersUrl}{url router=PKPApplication::ROUTE_COMPONENT context=$editContext->getPath() component="grid.settings.user.UserGridHandler" op="fetchGrid" escape=false}{/capture}
