 ...
 * @see Citation
 *
 * @brief Class modeling a described resource (DCMI abstract model) or subject
 *  (RDF). This class and its children provide meta-data (DCMI abstract
 *  model: statements of property-value pairs, RDF: assertions of predicate-
 *  object pairs) about PKP application entities.
 *
...
class MetadataRecord {
	...
	/**
	 * @var array meta-data statements about the resource
	...
	function addStatement($propertyName, &$value, $replace = true, $locale = null) {
		...
		switch ($property->getCardinality()) {
			...
			case METADATA_PROPERTY_CARDINALITY_MANY:
				if (isset($existingValue) && !$replace) {
					...
					// NB: existingValue is a reference
		...
		// Add the value
		if ($translated) {
			if (!isset($this->_statements[$propertyName])) {
				$this->_statements[$propertyName] = array();
			}
			$this->_statements[$propertyName][$locale] =& $newValue;
		} else {
			$this->_statements[$propertyName] =& $newValue;
		}
	...
	function removeStatement($propertyName) {
		...
		if (isset($propertyName) && isset($this->_statements[$propertyName])) {
			unset($this->_statements[$propertyName]);
			return true;
		}
	...
	function &getStatement($propertyName, $locale = null) {
		...
		$value = null;
		if ($translated) {
			if (isset($this->_statements[$propertyName][$locale])) {
				$value =& $this->_statements[$propertyName][$locale];
			}
		} else {
			if (isset($this->_statements[$propertyName])) {
				$value =& $this->_statements[$propertyName];
			}
		}
		return $value;
	}
