...
| U4 | ORCID integration | OJS OMP OPS | M | done | Spec verified; 9 tests OJS, 8 OMP, 9 OPS incl. 2 absence (+setup each), green ×2 + post-fold confirm; register 6🐞+5❓+2✅ (A5 Assistant false-success + A1 silent-close/over-offer are the headliners); claim check 40 claims 0 wrong; OAuth/deposit legs code-anchored (egress); skill rule-7 job_runner note fixed (self-healing); maintainer review pending |
