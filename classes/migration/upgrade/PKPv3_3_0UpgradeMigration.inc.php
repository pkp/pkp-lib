...
class PKPv3_3_0UpgradeMigration extends Migration
{
    ...
    private function _migrateSubmissionFiles()
    {
        ...
        switch (DB::getDriverName()) {
            case 'mysql':
                DB::unprepared("UPDATE event_log_settings elsr JOIN event_log_settings elsf ON (elsr.log_id = elsf.log_id AND elsf.setting_name='fileId') JOIN submission_files sf ON (sf.revision = elsr.setting_value AND sf.file_id = elsf.setting_value) SET elsr.setting_value = sf.new_file_id WHERE elsr.setting_name='fileRevision'");
                break;
            case 'pgsql':
                DB::unprepared("UPDATE event_log_settings elsr SET setting_value = sf.new_file_id FROM event_log_settings elsf, submission_files sf WHERE elsr.log_id = elsf.log_id AND elsf.setting_name='fileId' AND sf.revision = elsr.setting_value::INTEGER AND sf.file_id = elsf.setting_value::INTEGER AND elsr.setting_name='fileRevision'");
                break;
            default:
