	...
	{fbvFormArea id="categoryDetails"}
		...
		{fbvFormSection title="grid.category.path" required=true for="path"}
			{capture assign="instruct"}
				{url router=$smarty.const.ROUTE_PAGE page=$catalogPage op="category" path="path"}
		...
		{if $image}
			{fbvFormSection}
				...
				<img class="pkp_helpers_container_center" height="{$image.thumbnailHeight}" width="{$image.thumbnailWidth}" src="{url router=$smarty.const.ROUTE_PAGE page=$catalogPage op="thumbnail" type="category" id=$categoryId}" alt="{$altTitle|escape}" />
			{/fbvFormSection}
		{/if}
