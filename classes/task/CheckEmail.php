...
class CheckEmail extends ScheduledTask
{
    ...
    protected function handleDiscussionResponse(Imap $imap, string $messageId, array $headers, array &$messageParts = null): bool
    {
        ...
        // skip OJS-generated discussion emails that land in the monitored inbox
        $currentMessageIdHeader = null;
        foreach ($normalizedHeaders as $key => $value) {
            if (
                $currentMessageIdHeader === null
                && ($key === 'message-id' || $key === 'message_id')
            ) {
                $currentMessageIdHeader = is_array($value) ? reset($value) : $value;
            }
        }

        $currentMessageId = $this->normalizeMessageId($currentMessageIdHeader);

        if ($currentMessageId) {
            $existingSentNote = Note::withMessageId($currentMessageId)->first();
            ...
            if ($existingSentNote) {
                $this->dbg('message ' . $messageId . ' is an OJS-generated discussion email; skipping');
                return true;
            }
        }
        ...
        //Find message-id linkage headers and map to Note::messageId
        ...
        // Extract candidate emails from all sender headers
        $emailCandidates = [];
        ...
        $this->dbg('message ' . $messageId . ' matched userId=' . $user->getId() . ' email=' . $matchedEmail);
         ...
         // Find a text/plain part for the reply body
        $parts = $messageParts ?? $imap->getMessageParts($messageId);
        $messageParts = $parts;
