	...
	<p>
		...
		{else}
			...
			{elseif $nlm30PublicationType == 'journal'}
				{$nlm30ArticleTitle}. {$mainTitle}{if $subTitle} {$subTitle}{/if} {$nlm30Date|date_format:'%Y %b'}
			...
			{elseif $nlm30PublicationType == 'conf-proc'}
				...
				{if $nlm30ConfLoc}{$nlm30ConfLoc}: {/if}{$nlm30Date|truncate:4:''}
			{/if}
			{if $nlm30DateInCitationContentTypeAccessDate} [cited {$nlm30DateInCitationContentTypeAccessDate|date_format:'%Y %b %e'}]{/if}.
