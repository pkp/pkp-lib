...
class SubmissionFileDAODelegate extends DAO {
	...
	function updateObject($submissionFile, $previousFile) {
		...
		$this->update(
			...
			array(
				...
				$submissionFile->getDirectSalesPrice(),
				$submissionFile->getSalesType(),
				(int)$submissionFile->getSubmissionSettingsRevision(),
				(int)$submissionFile->getHideFileRevisions(),
	...
	function fromRow($row) {
		...
		$submissionFile->setSubmissionSettingsRevision($row['submission_settings_revision']);
		$submissionFile->setHideFileRevisions($row['hide_file_revisions']);
