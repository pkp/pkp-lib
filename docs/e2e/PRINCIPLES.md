...
`scenario-processor-audit.md` (append-only ledger).
...
- **A2 — Scenario builders must be accurate.** A seeded scenario leaves the
  ...
  builder change requires a parity entry in `scenario-processor-audit.md`
  before it merges.
...
(LIVE surface) with their parity rationale in `scenario-processor-audit.md` —
not here.
...
`scenario-processor-audit.md`; product findings → the feature spec's Findings
