...
class PKPJatsController extends PKPBaseController
{
     ...
     *
     * This endpoint does not require authentication but validates:
     * - Publication exists
     * - Publication is published (STATUS_PUBLISHED)
     * - jatsPublicVisibility setting is true
     *
     * The Response is cached for 24 hours to prevent DDOS.
     */
    public function publicDownload(Request $illuminateRequest): Response|JsonResponse
    {
        ...
        if (!$publication || $publication->getData('submissionId') !== $submissionId) {
            return response()->json([
                'error' => __('api.404.resourceNotFound'),
            ], Response::HTTP_NOT_FOUND);
        }
        ...
        $filename = 'article-' . $submissionId . '-jats.xml';
        if ($submission) {
            $urlPath = $publication->getData('urlPath') ?? $submissionId;
