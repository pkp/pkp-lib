...
class Repository
{
    public function addContributorRoles(array $newRoleIdentifiers, int $contributorId, ?int $contextId = null): void
    {
        $contextId ??= Application::get()->getRequest()->getContext()->getId();
        $roleIds = ContributorRole::withContextId($contextId)
            ->withIdentifiers($newRoleIdentifiers)
            ->select(['contributor_role_id'])
            ->get()
            ->pluck('contributor_role_id');

        // Disallow removal of all roles
    ...
    /**
     * Get all contributor's contributor roles with locale translations or just values.
     */
    public function getContributorRolesByContributorId(int $contributorId, bool $withTranslations = true): array
    {
            ...
            ->when(
                ...
                fn ($collection) => $collection
                    ->mapWithKeys(fn (ContributorRole $role) => [
                        $role->contributorRoleIdentifier => $role->name
                    ]),
                fn ($collection) => $collection
                    ->map(fn (ContributorRole $role): string => $role->contributorRoleIdentifier)
                    ->values()
            )
            ->toArray();
    }
